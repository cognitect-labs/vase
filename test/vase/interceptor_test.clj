...
(deftest stock-interceptors
  (testing "attach-received-time"
    (is (some-> (run-interceptor (attach-received-time "example.v1")) :request :received-time)))

  (testing "attach-request-id"
    (testing "default request id is supplied"
      (is (some-> (run-interceptor (attach-request-id "example.v1")) :request :request-id)))

    (testing "an explicit request id is returned"
      (let [original-id  "1234554321"
            ctx          (new-ctx "vaserequest-id" "1234554321")
            resulting-id (some-> (run-interceptor ctx (attach-request-id "example.v1")) :request :request-id)]
        (is (= original-id resulting-id)))))

  (testing "forward-headers"
    (let [fh              (forward-headers ["vaserequest-id" "custom-header"] "example.v1")
